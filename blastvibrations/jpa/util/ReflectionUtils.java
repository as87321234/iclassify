...
public class ReflectionUtils {
    ...
    public static void cloneSkipNull(final Object fromObj, final Object toObj,
	    String[] excludedProperties) {

	if (fromObj.getClass() != toObj.getClass()) {

	    throw new RuntimeException(
		    "Source  and Destination class are incompatible, using the cloneLoosy");
	...
	try {
		...
		if (pd != null
			&& pd.getDisplayName().equals(
				propertyDescriptor.getDisplayName())
			&& !pd.getDisplayName().equals("class")
			&& !Arrays.asList(excludedProperties).contains(
				pd.getDisplayName())) {
			...
			final boolean hasReaderMethodSkipCloneAnnocationPresent = (pd
				.getReadMethod() == null ? false : pd
				.getReadMethod().isAnnotationPresent(
					ReflectionDontClone.class));
			final boolean hasWriterMethodSkipCloneAnnocationPresent = propertyDescriptor
				.getWriteMethod().isAnnotationPresent(
					ReflectionDontClone.class);

			if (!hasReaderMethodSkipCloneAnnocationPresent
				&& !hasWriterMethodSkipCloneAnnocationPresent) {
    ...
    public static void clone(final Object fromObj, final Object toObj,
	    String[] excludedProperties) {

	if (fromObj.getClass().getSuperclass().getCanonicalName()
		.equals(toObj.getClass().getCanonicalName())) {
	...
	try {
	    ...
	    Log.error("ReflectionUtils.clone cannot introspec exception with field name '"
		    + "'" + fieldNameProcessing);
	    e.printStackTrace();
	} catch (final IllegalArgumentException e) {
    ...
    public static void cloneLoosy(final Object fromObj, final Object toObj,
	    String[] excludedProperties) {
    ...
    public static void cloneLoosySkipNull(final Object fromObj,
	    final Object toObj, String[] excludedProperties) {
    ...
    public static void cloneLoosyAutoConversion(final Object fromObj,
	    final Object toObj, String[] excludedProperties) {
	...
	try {
		...
		if (pd != null
			...
			if (!hasReaderMethodSkipCloneAnnocationPresent
			    ...
			    propertyDescriptor.getWriteMethod().invoke(
				    toObj,
				    TypeConverter.generic(propertyDescriptor
					    .getPropertyType(), o));
			}
		    }
		}
    ...
    public static void cloneLoosyAutoConversion(final Object srcObj,
	    final Object dstObj) {
    ...
    /*
     * Clone Structure A in B only looking at String properties. if the source
     * is not null and the destination is not null, append the string a into B
     * and separate with comma.
     */

    public static void cloneAppend(final Object fromObj, final Object toObj,
	    String[] excludedProperties) {
	...
	try {
		...
		if (pd != null
			...
			if (!hasReaderMethodSkipCloneAnnocationPresent
			    ...
			    Class<?> type = propertyDescriptor
				    .getPropertyType();

			    if (String.class.isAssignableFrom(type)) {

				String label = (String) pd.getReadMethod()
					.invoke(toObj);

				String newLabel = (String) propertyDescriptor
					.getReadMethod().invoke(fromObj);
    ...
    /*
     * Clone Structure A in B only looking at the boolean properties. if the
     * source is true and the destination is false replace the destination,
     * otherwise ignore.
     */
    public static void cloneBooleanTrue(final Object fromObj,
	    final Object toObj, String[] excludedProperties) {
	...
	try {
		...
		if (pd != null
			...
			if (!hasReaderMethodSkipCloneAnnocationPresent
				...
				Boolean currentState = (Boolean) pd
					.getReadMethod().invoke(toObj);

				if (currentState == null) {
				    currentState = false;
				}

				Boolean newState = (Boolean) propertyDescriptor
					.getReadMethod().invoke(fromObj);
